...
class Primer3Wrapper:
    ...
    def design_primers(self, sequence: str, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Probe Design
        # ARCHITECTURE NOTE:
        #   - RAA workflow: probes are found MANUALLY by find_exo_probe() AFTER Primer3 returns
        #     primers. We must NOT send any PRIMER_INTERNAL_* settings to Primer3 because RAA
        #     probes (46-52nt) exceed Primer3's hard-coded limit (~36nt), causing a fatal error.
        #   - qPCR workflow: Primer3 handles short probes (18-30nt) natively.
        probe_params = params.get('probe')
        if probe_params and probe_params.get('enabled', True):
            if workflow_type == 'raa':
                # RAA: Primer3 picks primers only. Probe is handled downstream.
                p3_settings['PRIMER_PICK_INTERNAL_OLIGO'] = 0
            else:
                # qPCR: Let Primer3 handle short probes
                p3_settings.update({
                ...
                # Task 4.7 Mishybridization library for probes (qPCR only)
